...
create table items (
	...
	itemHandlerBeanName varchar(150),
	name varchar(150) not null,
	parentItemName varchar(150),
	parentItemType varchar(150),
	template_id bigint,
...
CREATE VIEW name_value_pairs AS
  ...
  FROM property_definition prop, items it WHERE prop.item_id=it.id
   and prop.name not in ('TemplateName','SecuritySameAsParent','behaviors', 'bundleName', 'client-template', 'ItemRef', 'bd_seo_canonical', 'generatedUrl')
  UNION
